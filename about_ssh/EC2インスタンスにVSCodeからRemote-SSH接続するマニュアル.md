...
続いて、新しく開いた VSCode のターミナルで以下のコマンドで次のスクリプトを実行します。実行に 10 分以上かかります。

```bash:ターミナルので実行コマンド
curl -sS https://raw.githubusercontent.com/techboost-curriculum-updaters-new/scripts/master/about_ssh/setup.sh | sh; exec $SHELL -l
```

### Java コースの AWS デプロイの場合
